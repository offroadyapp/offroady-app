...
export async function generateStaticParams() {
  ...
  for (const item of blogItems) {
    if (item.lang === 'en') slugs.add(item.translation.slug);
  }
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const resolved = await resolveBlogContent('en', slug);
  if (!resolved) return {};
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.offroady.app/blog/en/${slug}`,
      languages: {
    ...
    openGraph: {
      title: resolved.seoTitle,
      description: resolved.seoDescription,
      url: `https://www.offroady.app/blog/en/${slug}`,
      siteName: 'Offroady',
...
export default async function EnBlogPostPage({ params }: Props) {
  const { slug } = await params;
  return <BlogDetailContent lang="en" slug={slug} />;
}
